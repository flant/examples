...
echo "export ROOT_RELEASE='${ROOT_RELEASE:="1.0"}'"
export ROOT_VERSION=$(multiwerf available-releases ${ROOT_RELEASE} | head -n 1 | awk '{print $1}')
echo "export ROOT_VERSION='$ROOT_VERSION'"
export CHANNELSJSON=$(multiwerf available-releases ${ROOT_RELEASE} -o json)
echo "export CHANNELSJSON='$CHANNELSJSON'"
export CHANNELSDEVJSON=$(echo $CHANNELSJSON | jq -cM '.orderedReleases = ["master"] + .orderedReleases')
echo "export CHANNELSDEVJSON='$CHANNELSDEVJSON'"
